...
def ppt_to_pdf(input_file, output_file):
    ...
    deck.SaveAs(output_file, 32)  # 32 stands for pdf format
    deck.Close()
    powerpoint.Quit()
...
PRINT_STACK_TRACE = os.getenv(
    'PRINT_STACK_TRACE', '1').lower() in ('true', 't', '1')
try:
...
bot = commands.Bot(command_prefix=PREFIX, intents=discord.Intents(
...
async def queue(ctx: commands.Context, *args):
    ...
    if queue == None:
        ...
        queue_str = ''.join(
            map(title_str, enumerate([i[1]["title"] for i in queue])))
...
async def skip(ctx: commands.Context, *args):
    ...
    voice_client = get_voice_client_from_channel_id(
    ...
    with yt_dlp.YoutubeDL({'format': 'worstaudio',
        ...
        try:
            ...
            try:
                ...
                connection = get_voice_client_from_channel_id(
...
def after_track(error, connection, server_id):
    ...
    # check that the same video isn't queued multiple times
    ...
    try:
        ...
        asyncio.run_coroutine_threadsafe(
            safe_disconnect(connection), bot.loop).result()
...
async def on_command_error(event: str, *args, **kwargs):
    ...
    sys.stderr.write(
        f'{type_}: {value} raised during {event}, {args=}, {kwargs=}')
    sp.run(['./restart'])
